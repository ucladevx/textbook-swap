...
exports.add_book = function(user, book, next){
    pg.connect(utilities.database_url, function(err, client, done){
        ...
        if (err){
            logger.error("Error connection to client while querying wish_list table: ", err);
            return next(utilities.wish_list_errors.DB_CONNECTION_ERROR);
        }
        ...
        client.query("SELECT COUNT(user_id) FROM wish_list WHERE user_id=$1::VARCHAR AND book_id=$2::INTEGER", [user, book], function(err, result){
            if (err){
                logger.error("Error querying table wish_list", err);
                return next(utilities.wish_list_errors.DB_QUERY_ERROR);
            }
            ...
            if(result.rows[0].count == 0){
                client.query("INSERT INTO wish_list (user_id, book_id) VALUES ($1::VARCHAR, $2::INTEGER)", [user, book], function(err, result){
                    if (err){
                        logger.error("Error inserting into wish_list table", err);
                        return next(utilities.wish_list_errors.DB_QUERY_ERROR);
                    }

                    return next(utilities.wish_list_errors.DB_SUCCESS);
                });
            }
            else{
                logger.error("UserID, BookID association already exists in wish_list table");
                return next(utilities.wish_list_errors.WISH_LIST_BOOK_ALREADY_EXISTS);
            }
        });
    });
};
...
exports.get_wanted_books = function(user, next){
    pg.connect(utilities.database_url, function(err, client, done){
        ...
        client.query("SELECT book_id FROM wish_list WHERE user_id=$1::VARCHAR", [user], function(err, result){
            ...
            return next(utilities.wish_list_errors.DB_SUCCESS, result.rows);
        });
    });
};
