        ...
        beforeEach(function() {
            pg.connect(utilities.database_url, function(err, client, done){
                ...
                client.query("INSERT INTO found_trades VALUES (1, '111', 123, '222', 124,'P'); \
                    ...
                    [], function(err, result){
                        if(err){
                            logger.error("Error inserting into test found_trades database", err);
                            done(utilities.found_trades_errors.DB_QUERY_ERROR);
                        }
                        done(utilities.found_trades_errors.DB_SUCCESS);
                    });
            });
        });
        ...
        afterEach(function() {
            pg.connect(utilities.database_url, function(err, client, done){
                ...
                client.query("TRUNCATE TABLE found_trades",
                    [], function(err, result){
                        if(err){
                            logger.error("Error truncating test found_trades database", err);
                            done(utilities.found_trades_errors.DB_QUERY_ERROR);
                        }
                        done(utilities.found_trades_errors.DB_SUCCESS);
                    });
        ...
        describe('#automatic_reject_old_trades()', function() {
            it('should set all statuses to R except for trade_id=1', function(done) {
                foundTrades.automatic_reject_old_trades(function(err){
                    pg.connect(utilities.database_url, function(err, client, done){
                        client.query("SELECT trade_id FROM found_trades WHERE status='P'",
                            [], function(err, result){
                                if(err){
                                    logger.error("Error rejecting two day old pending trades database while testing", err);
                                    done(utilities.found_trades_errors.DB_QUERY_ERROR);
                                }
                                logger.debug("result", result.rows);
                                //check that trade_id = 1
                                assert.equal(1, result.rows[0]['trade_id']);
